...
const { open } = require("sqlite"); // Importing the open function from the sqlite module
const sqlite3 = require("sqlite3"); // Importing the sqlite3 module
...
app.use(express.json()); // Using the JSON middleware for parsing request bodies
let database = null; // Initializing the database variable
let dbFilePath = path.join(__dirname, "./covid19IndiaPortal.db"); // Setting the path for the database file
...
connectServerWithDb(); // Calling the function to connect the server with the database
...
const authToken = (req, res, next) => {
  ...
  if (authHeader === undefined) {
    ...
    jwt.verify(jwtToken, "SECRET_KEY_DEV", async (error, payload) => {
      if (error) {
        res.status(401);
        res.send(`Invalid JWT Token`);
      } else {
...
app.post("/login/", async (req, res) => {
  ...
  if (checkInDatabase === undefined) {
    ...
    if (!isPasswordMatched) {
      ...
      let jwtToken = jwt.sign(payload, "SECRET_KEY_DEV");
      res.send({ jwtToken });
    }
  }
});
